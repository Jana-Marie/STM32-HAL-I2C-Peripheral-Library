...
void HAL_I2C_AddrCallback(I2C_HandleTypeDef *hi2c, uint8_t direction, uint16_t addrMatchCode) {
	switch (direction) {
		...
		case I2C_DIRECTION_RECEIVE:
			...
			if (HAL_I2C_Slave_Seq_Transmit_IT(hi2c, &i2c_memory[i2c_data.registerAddress++], 255, I2C_NEXT_FRAME) != HAL_OK) {
				I2C_Error_Handler();
			}
		break;
    default:
...
void HAL_I2C_SlaveRxCpltCallback(I2C_HandleTypeDef *hi2c) {
	switch (i2c_data.transferState) {
		case getRegisterAddress:
			...
			if (HAL_I2C_Slave_Seq_Receive_IT(hi2c,&i2c_memory[i2c_data.registerAddress++], 1, I2C_FIRST_FRAME) != HAL_OK) {
				I2C_Error_Handler();
			}
		break;
		case getData:
			if (HAL_I2C_Slave_Seq_Receive_IT(hi2c, &i2c_memory[i2c_data.registerAddress++], 255, I2C_FIRST_FRAME) != HAL_OK) {
				I2C_Error_Handler();
			}
		break;
    default:
